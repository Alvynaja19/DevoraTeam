        ...
        <form method="POST" action="{{ route('members.store') }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="flex flex-wrap gap-4">
                        <label class="flex items-center gap-2 cursor-pointer group">
                            <input type="radio" name="status" value="siswa" class="w-4 h-4 text-green-600 focus:ring-green-500 border-slate-300" {{ old('status', 'siswa') === 'siswa' ? 'checked' : '' }}>
                            <span class="text-sm text-slate-700 group-hover:text-slate-900 transition-colors">Siswa</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer group">
                            <input type="radio" name="status" value="guru" class="w-4 h-4 text-green-600 focus:ring-green-500 border-slate-300" {{ old('status') === 'guru' ? 'checked' : '' }}>
                            <span class="text-sm text-slate-700 group-hover:text-slate-900 transition-colors">Guru</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer group">
                            <input type="radio" name="status" value="masyarakat umum" class="w-4 h-4 text-green-600 focus:ring-green-500 border-slate-300" {{ old('status') === 'masyarakat umum' ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="kelas" id="kelas" value="{{ old('kelas') }}" placeholder="Contoh: X-1"
